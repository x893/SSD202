...
//    }
    ...
    if (!IS_ARG_AVAILABLE_SCRIPT_FILE(argc))
    {
        ...
        memset((void * )GOP_DISP_ADDR, 0x00, width*height*4);
        DrawProgressBar((width - PROGRESS_BAR_WIDTH) / 2,((height - PROGRESS_BAR_HEIGHT) / 5) - 5, 0, 0);
    }
#endif
...
//#endif
        ...
        while ((next_line = get_script_next_line(&script_buf)) != NULL)
        {
            ...
            if (u32Percentage != 0)
            {
                ...
                DrawProgressBar((width - PROGRESS_BAR_WIDTH) / 2,((height - PROGRESS_BAR_HEIGHT) / 5) - 5,u32Percentage, 0);
...
int do_sdstar     (cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
    ...
    printf("do_sdstar-----------> total_cmd=%d\n", total_cmd);
    if(total_cmd > 0)
    {
        memset(tmp,0,sizeof(tmp));
        snprintf(tmp, sizeof(tmp) - 1,"gpio output 4 1");
	run_command(tmp,0);
...
int do_usbstar     (cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	...
	printf("do_usbstar-----------> total_cmd=%d\n", total_cmd);
	if (total_cmd > 0){
		//run_command("gpio output 4 1", 0);														
		memset(tmp,0,sizeof(tmp));
        	snprintf(tmp, sizeof(tmp) - 1,"gpio output 4 1");
        	run_command(tmp,0);
...
//#endif
	    ...
	    u32Percentage += 5;
	    printf("----------- want to DrawProgressBar %d\n", u32Percentage);
            if (u32Percentage != 0)
            {
	        printf("----------- DrawProgressBar \n");
#ifdef CONFIG_MS_SHOW_LOGO
	        printf("----------- DrawProgressBaring \n");
                //DrawProgressBar((_getPanelWidth() - PROGRESS_BAR_WIDTH) / 2,((_getPanelHeight() - PROGRESS_BAR_HEIGHT) / 5) - 5,u32Percentage, 0);
                DrawProgressBar((width - PROGRESS_BAR_WIDTH) / 2,((height- PROGRESS_BAR_HEIGHT) / 5) - 5,u32Percentage, 0);
                //sprintf(buf_percent, "%d", u32Percentage);
                //UPDPrintLineSize(buf_percent, COLOR_WHITE, 2, POS_CENTER, 6);
...
int do_macburn (cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
    ...
    memset(tmp,0,sizeof(tmp));
    //snprintf(tmp, sizeof(tmp) - 1, "setenv macaddr %s", mac_addr);
    snprintf(tmp, sizeof(tmp) - 1, "setenv -f ethaddr %s", mac_addr);
    ret = run_command(tmp, 0);
    if (ret != 0) {
        printf("setenv macaddr error!\n");
        return -1;
    }
    run_command("saveenv", 0);
